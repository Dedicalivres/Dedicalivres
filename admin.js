...
(function () {
  ...
  /*
    Si ta table de visites porte un autre nom, change uniquement cette ligne.
    Exemples possibles : "site_visits", "visits", "analytics_events".
  */
  ...
  /*
    Important :
    Cette URL doit aussi être ajoutée dans Supabase :
  ...
  */
  ...
  async function init() {
    ...
    try {
      ...
      /*
        Toujours écouter les changements Auth, même en recovery.
        Sinon Supabase peut créer la session recovery après init()
        et le formulaire nouveau mot de passe ne fonctionnera pas.
      */
  ...
  function bindEvents() {
    ...
    /*
      ...
      si le bouton existe mais que le listener direct ne s’attache pas
      à cause d’un cache ou d’un rendu tardif, ce listener global capte le clic.
    */
    document.addEventListener("click", function (event) {
      const target = event.target;

      if (target && target.id === "forgot-password-btn") {
        event.preventDefault();
        handleForgotPassword();
        return;
      }

      if (target && target.dataset.closeModal === "true") {
        closeEditModal();
      }

      if (target && target.dataset.closeConfirm === "true") {
        closeConfirmModal();
      }
    });
  ...
  async function handleLogin(event) {
    ...
    const button = event ? event.submitter : null;
    setLoadingButton(button, true, "Connexion...");
  ...
  function renderStats() {
    ...
    const pending = all.filter(function (event) {
      return getEventStatus(event) === "pending";
    }).length;
    const validated = all.filter(function (event) {
      return event.validated === true;
    }).length;
    const rejected = all.filter(function (event) {
      return event.rejected === true;
    }).length;
    const featured = all.filter(function (event) {
      return event.featured === true;
    }).length;
    const verified = all.filter(function (event) {
      return event.verified === true;
    }).length;

    setText(els.statTotal, total);
    setText(els.statPending, pending);
    setText(els.statValidated, validated);
    setText(els.statRejected, rejected);
    setText(els.statFeatured, featured);
    setText(els.statVerified, verified);
  }
  ...
  if (!config || !config.supabaseUrl || !config.supabaseAnonKey) {
    showFatalError("Configuration Supabase manquante. Vérifie le fichier config.js.");
    throw new Error("Configuration Supabase manquante.");
  }

  if (!window.supabase || !window.supabase.createClient) {
    showFatalError("Supabase JS v2 est introuvable.");
    throw new Error("Supabase JS v2 introuvable.");
  }
  ...
  async function init() {
    ...
    state.isRecoveryMode = window.location.hash.includes("type=recovery");
  ...
  function getErrorMessage(error) {
    ...
    if (message.includes("Email rate limit exceeded") || message.includes("email rate limit exceeded")) {
      return "Trop d’emails envoyés en peu de temps. Attends avant de redemander un lien.";
    }
  ...
  function showLogin() {
    ...
    els.adminView.classList.add("is-hidden");
  ...
  function showAdmin() {
    ...
    els.adminView.classList.remove("is-hidden");
  ...
  async function handleEditSubmit(event) {
    ...
    try {
      ...
      const patch = {
        ...
        lng: nullableNumber(valueOf(els.editLng)),
        source_label: nullableString(valueOf(els.editSourceLabel)),
        validated: Boolean(els.editValidated.checked),
        rejected: Boolean(els.editRejected.checked),
        featured: Boolean(els.editFeatured.checked),
        verified: Boolean(els.editVerified.checked)
      };
  ...
  els.viewList.classList.toggle("is-hidden", viewName !== "list");
  els.viewMap.classList.toggle("is-hidden", viewName !== "map");
  ...
  state.map.off("popupopen");
  state.map.on("popupopen", function (popupEvent) {
    const button = popupEvent.popup.getElement().querySelector("[data-map-edit-id]");
    if (!button) return;
  ...
  if (state.filteredEvents.filter(hasCoordinates).length > 300) {
    showMessage(
      els.globalMessage,
      "info",
      "Carte optimisée : seuls les 300 premiers événements géolocalisés sont affichés. Utilise les filtres pour affiner."
    );
  }
}
  ...
  function getErrorMessage(error) {
    ...
    if (message.includes("Could not find") && message.includes("schema cache")) {
      return "Une colonne semble absente dans Supabase. Le code V3 évite les colonnes indisponibles quand c’est possible.";
    }
