(function () {
  ...
  function renderMapMarkers(events) {
    ...
    if (validCoords.length === 1) {
      ...
      map.fitBounds(L.latLngBounds(validCoords.map((event) => [Number(event.lat), Number(event.lng)])), { padding: [25, 25] });
    } else {
  ...
  async function uploadImage(file) {
    ...
    const { error } = await supabaseClient.storage.from("event-images").upload(fileName, file, {
      cacheControl: "3600",
      upsert: false
    });
  ...
  function setLoadingState() {
    if (eventsGrid) eventsGrid.innerHTML = `<article class="empty-state"><div class="loader"></div><p>Chargement des événements...</p></article>`;
  }

  function setErrorState(message) {
    if (eventsGrid) eventsGrid.innerHTML = `<article class="empty-state"><p>${escapeHtml(message)}</p></article>`;
  }
  ...
  function slugify(value) {
    return normalize(value).replace(/[^a-z0-9]+/g, "-").replace(/^-|-$/g, "") || "evenement";
  }
