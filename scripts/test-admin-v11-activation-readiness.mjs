...
assert.doesNotMatch(shell, /V11 · préactivation/);
assert.match(shell, /releaseState\.dataset\.v11ReleaseState = "active"/);
assert.match(shell, /releaseState\.textContent = "V11 · active"/);
...
assert.match(v11, /Le retrait réversible est disponible pour les administrateurs/);
assert.match(config, /adminV11CommunityArchiveEnabled: true/);
assert.match(shell, /!isV11CommunityArchiveEnabled\(\)/);
